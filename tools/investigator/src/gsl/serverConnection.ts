...
export class ServerConnection extends EventEmitter {
    ...
    connect() {
        ...
        if (this.debug === true) {
            this.console.log("Connection#connect", JSON.stringify(this.options));
        }
        if (this.key === null) {
    ...
    private processBuffer(newLine: string) {
        ...
        while (eol > -1) {
            const line = this.receiveBuffer.substring(last + newLine.length, eol);
    ...
    private socketData(data: Buffer) {
        const text: string = data.toString("ascii");
        if (this.debug === true) {
