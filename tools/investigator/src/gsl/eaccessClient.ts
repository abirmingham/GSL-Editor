...
const PINNED_CA = readFileSync(join(__dirname, "eaccess.play.net.pem"));
...
export class EAccessClient extends EventEmitter {
    ...
    connect() {
        if (this.status === STATUS_NEW) {
            this.socket = tls_connect(
                this.port,
                this.host,
                { ca: PINNED_CA, checkServerIdentity: () => undefined },
                () => {
                    ...
                    if (certificate.pubkey.toString("hex").toUpperCase() !== PUBKEY.toUpperCase()) {
                        this.console.log(
                            `Certificate pubkey ${certificate.pubkey.toString("hex")} does not match expected value.`,
                        );
                        this.socket.end();
                        this.emit(
                            EVENT_ERROR,
            ...
            if (this.debug) {
                this.socket.on("data", (data) =>
                    this.console.log("EAccess:socket:data", JSON.stringify(data.toString("ascii"))),
                );
            }
            ...
            this.socket.on("close", (errored) => {
                if (this.debug) {
                    this.console.log("EAccess:socket:close", errored ? "(with error)" : "(ok)");
                }
    ...
    private hashPassword(password: string): Buffer {
        ...
        const hash = Buffer.alloc(pw.length);
        let n, nn;
    ...
    private a11n_response(data: Buffer) {
        ...
        const reply = response.split(RX_TAB_NL);
        const [_code, account, result, key, owner] = reply;
        if (this.debug) {
        ...
        switch (result) {
            ...
            default:
                if (this.debug) {
                    this.console.log("EAccess unhandled A response: %s", response);
                }
    ...
    private receiveGameList(data: Buffer) {
        ...
        for (let n = 1; n < reply.length - 1; n += 2) {
            const code = reply[n].trim();
            const name = reply[n + 1].trim();
    ...
    private selectGame_N_response(data: Buffer) {
        ...
        const game = (this.selectedGame = this.games[this.selectedGameCode]);
        const [env, storm, trial] = type.split("|");
    ...
    private selectGame_C_response(data: Buffer) {
        const response = data.toString("ascii");
        const reply = response.split(RX_TAB_NL);
        const [code, characterCount, characterSlots] = reply;
        if (this.debug) {
    ...
    static quickLogin(
        ...
        const attempt = () =>
            this.quickLoginImpl(account, password, game, character, mode, abortSignal);
    ...
    private static quickLoginImpl(
        ...
        const options = { debug: this.debug, console: this.console };
        return new Promise((resolve, reject) => {
            ...
            const onAbort = () => {
                ...
                rejectOnce(
                    createQuickLoginError("QuickLoginCancelledError", "Quick login cancelled."),
                );
            };
            ...
            client.once(
                EVENT_CHARS,
                (characters: CharacterOptionCollection, codes: Array<string>) => {
                    const resolvedCode =
                        codes.find((c) => c === character) ??
                        codes.find(
                            (c) => characters[c].name.toLowerCase() === character.toLowerCase(),
                        );
                    if (!resolvedCode) {
                        client.close();
                        rejectOnce(
    ...
    static console: { log: (...args: any) => void } = { log() {} };
    ...
    static login(
        ...
        return new Promise((resolve, reject) => {
            ...
            const client = new this(options);
            client.once(EVENT_CLOSE, () => reject(new Error("EAccess connection closed.")));
...
class GameOptionChoice {
    ...
    select(code: string): Promise<CharacterOptionChoice> {
        return new Promise((resolve, reject) => {
            this.client.once(EVENT_CLOSE, () => reject(new Error("EAccess connection closed.")));
...
class EAccessError extends Error {
    ...
    constructor(code: string) {
        ...
        const message: string =
            errorCodes[code] || `An unknown error code ${code} has been encountered.`;
        super(message);
        this.code = code;
    }
}
