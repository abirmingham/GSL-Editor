...
class TaskQueueProcessor {
    /** Number of milliseconds to wait before checking on the queue */
    private static CHECKUP_DELAY = 250
    ...
    scheduleTick(milliseconds: number): void {
        ...
        this.tickTimeout = setTimeout(() => this.tick(), milliseconds)
    }
    ...
    async tick(): Promise<void> {
        ...
        if (this.isProcessingTask) {
            this.scheduleTick(TaskQueueProcessor.CHECKUP_DELAY)
            return
        }
